...
sub _splitty {
    ...
    for (@r) {
        ...
        /^A\d\d$/
            and $_ = "http://dev.perl.org/perl6/apocalypse/$_.html"
            and next;

        /^E\d\d$/
            and $_ = "http://dev.perl.org/perl6/exegesis/$_.html"
            and next;

        /^S\d\d$/
            and $_ = "http://dev.perl.org/perl6/synopsis/$_.html"
            and next;

        /^\d+[A-Z]{4}$/
            and $_ = "/scribble.plp?scribblet=$_"
            and next;
        ...
        $_ = $self->_make_not_found_link($_);
    }
    return map { /,/ ? $self->_splitty($_) : (length $_ ? $_ : ()) } @r;
}
...
sub _make_not_found_link {
    my ( $self, $what ) = @_;
    return "http://search.cpan.org/perldoc?$what";
}
...
sub _make_tags {
    ...
    for ( @perldoc ) {
        $tags{$_} = "http://search.cpan.org/perldoc?$_";
        $tags{substr $_, 4} = "http://search.cpan.org/perldoc?$_";
    }

    $tags{$_} = "http://perldoc.perl.org/functions/$_.html"
        for @perlfunc;
...
CPAN::LinksToDocs - get links to http://search.cpan.org documentation by giving short "tags"
...
Returns a freshly baked C<CPAN::LinksToDocs> object. Takes one I<optional>
argument C<tags> which takes a hashref as a value. The keys of this
...
way you can link to custom made modules available on CPAN. B<Note:> the
module will B<NOT> check if there is any docs actually available in this
case (the non-matched tag case). If you want that functionality, consider
using L<CPAN::LinksToDocs::No404s> instead.
...
Below are the known tags for CPAN::LinksToDocs module:
...
L<http://tnx.nl/404>, L<CPAN::LinksToDocs::No404s>,
...
Please report any bugs or feature requests to C<bug-cpan-linkstodocs at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=CPAN-LinksToDocs>.  I will be notified, and then you'll
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=CPAN-LinksToDocs>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/CPAN-LinksToDocs>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/CPAN-LinksToDocs>

=item * Search CPAN

L<http://search.cpan.org/dist/CPAN-LinksToDocs>
